...
var exitTriggerSchema = new mongoose.Schema({
  variety: String,
  order_id: String,
...
let createExitTrigger = trigger => {
  ...
  exitTrigger.save(err => {
    if (err) {
      console.log(err, 'Error Storing Trigger to DB');
    }
  });
};

let updateExitTrigger = (trigger_id, updated_status) => {
